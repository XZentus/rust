...
impl fmt::Display for Expr {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match &self {
            //Expr{optype: Add,  => write!(f, "({}) + ({})", e1, e2),
            /*
            Sub => write!(f, "({} - {})", e1, e2),
            Mul => write!(f, "({} * {})", e1, e2),
            Div => write!(f, "({} / {})", e1, e2),
            */
            Expr{optype: op, left:  Some(box l),
                             right: Some(box r), ..}
                             if *op >= Add && *op <= Div => {
                              if l.optype == Add || l.optype == Sub ||
                                 l.optype == Num || l.optype == Arg {
                                  write!(f, "{} + ", l).unwrap();
                              }
                              else {
                                  write!(f, "({}) + ", l).unwrap();
                              }
                              if r.optype == Add || r.optype == Sub ||
                                 r.optype == Num || r.optype == Arg {
                                  write!(f, "{} + ", r)
                              }
                              else {
                                  write!(f, "({}) + ", r)
            ...
            Expr{optype: Sin, left: Some(box l), ..}    => write!(f, "sin({})", l),
            Expr{optype: Cos, left: Some(box l), ..}    => write!(f, "cos({})", l),
            Expr{optype: Tan, left: Some(box l), ..}    => write!(f, "tan({})", l),
            Expr{optype: Num, n, ..} => write!(f, "{}", n),
            Expr{optype: Arg, ..}    => write!(f, "x"),
            _ => write!(f, "DEFAULT"),
        }
    }
}
...
fn main() {
    ...
    train(&mut population, &points, 500);
    ...
    */
    let e = Expr{ optype: Add, left: Some(box Expr{ optype: Arg, left: None, right: None, n: 0.0}), right: Some(box Expr{ optype: Num, left: None, right: None, n: 10.0}), n: 0.0};
    match &e {
        Expr { right: Some(box Expr { n, .. }), .. } => println!("{:?}", n),
        _ => (),
